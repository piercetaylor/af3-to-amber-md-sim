...
These are the extended original workflow notes. The [current overview](../README.md) identifies the placeholders that require review before use.
...
Check the proposed histidine protonation states against the local hydrogen-bond network and intended conditions before topology building. A single protonation rule does not apply to every histidine.
...
After energy minimization, inspect the energy trend and the final structure. If a job fails, dependent jobs submitted with `afterok` will not start. Check the queue and cancel jobs left pending with an unsatisfied dependency if needed.
...
Throughput depends on the system size, GPU, and cluster configuration. Use the value reported by the current run to estimate completion time.
